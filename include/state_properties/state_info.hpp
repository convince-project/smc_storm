...
namespace state_info {
    ...
    inline bool checkNoInfo(StateInfoType const info) {
        return info ==  NO_INFO;
    }

    inline bool checkIsTerminal(StateInfoType const info) {
        return (info & IS_TERMINAL) != 0U;
    }

    inline bool checkBreakCondition(StateInfoType const info) {
        return (info & BREAK_CONDITION) != 0U;
    }

    inline bool checkSatisfyTarget(StateInfoType const info) {
