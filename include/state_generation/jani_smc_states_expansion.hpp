...
class JaniSmcStatesExpansion {
  ...
  private:
     ...
     * @param external_plugins description of the external plugins to use
     */
    void loadPlugins(const std::vector<model_checker::SmcPluginInstance>& external_plugins);
     ...
     * @param output_mapping The information about the plugin's output relation to the model's variables
     */
    ...
    /*!
     * @brief An internal data structure to handle plugin instances and related input expressions.
     */
    struct PluginHolder {
        // Plugin instance
        std::unique_ptr<smc_verifiable_plugins::SmcPluginBase> instance_ptr;
        // A map relating the plugin input to an expression
    ...
    std::vector<PluginHolder> _loaded_plugins;
