...
: _property_description(formula) {
    ...
    STORM_LOG_THROW(_generator_ptr, storm::exceptions::InvalidOperationException, "Cannot make a next state generator with provided model.");
...
template<typename StateType, typename ValueType>
void StateGeneration<StateType, ValueType>::initStateToIdCallback(samples::ExplorationInformation<StateType, ValueType>& exploration_information) {
    _state_to_id_callback = [&exploration_information, this](storm::generator::CompressedState const& state) -> StateType {
        StateType new_index = _state_storage_ptr->getNumberOfStates();

        // Check, if the state was already registered.
        std::pair<StateType, std::size_t> actual_index_bucket_pair = (_state_storage_ptr->stateToId).findOrAddAndGetBucket(state, new_index);
