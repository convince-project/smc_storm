...
BatchBuffer::BatchBuffer(const size_t n_threads, const size_t n_slots)
    : _n_slots{n_slots}, _results_buffer(n_threads) {
...
void BatchBuffer::addResults(const BatchResults& results, const size_t thread_id) {
    STORM_LOG_THROW(thread_id < _results_buffer.size(), storm::exceptions::IllegalArgumentException,
...
std::optional<std::vector<BatchResults>> BatchBuffer::getResults() {
    ...
    {
        std::lock_guard<std::mutex> lock(_buffer_mutex);
        if (std::any_of(_results_buffer.begin(), _results_buffer.end(), [](const auto& thread_results) { return thread_results.empty(); })) {
            return std::nullopt;
        }
